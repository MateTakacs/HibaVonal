...
export const useIssueSelection = (
  fetchIssueById,
  errorMessage = "A hibajegy részletei nem tölthetők be.",
) => {
  ...
  const loadSelectedIssue = useCallback(
    async (issueId) => {
      ...
      try {
        ...
        showErrorToast(
          createApiErrorMessage(error, errorMessage),
          "issue-detail-load",
        );
        return null;
      }
  ...
  const syncSelectionWithList = useCallback(
    (items) => {
      ...
      const stillExists = items.some(
        (item) => item.id === currentSelectedIssueId,
      );
  ...
  const refreshSelectedIssue = useCallback(
    async () => loadSelectedIssue(selectedIssueIdRef.current),
    [loadSelectedIssue],
  );
