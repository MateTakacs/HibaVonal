...
export const getHomeRouteForRole = (role) => {
  if (role === ROLES.USER) return APP_ROUTES.COLLEGIATE;
...
export const canRoleAccessPath = (role, pathName) => {
  ...
  if (role === ROLES.USER) return pathName.startsWith(APP_ROUTES.COLLEGIATE);
  if (role === ROLES.MAINTAINER)
    return pathName.startsWith(APP_ROUTES.MAINTAINER);
  if (role === ROLES.LEAD_MAINTAINER)
    return pathName.startsWith(APP_ROUTES.LEAD_MAINTAINER);
  if (role === ROLES.ADMIN) return pathName.startsWith(APP_ROUTES.ADMIN);
  return false;
};
