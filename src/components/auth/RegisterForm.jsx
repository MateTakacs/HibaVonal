...
const RegisterForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const message = err.response?.data?.message || "A regisztráció sikertelen.";
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
    ...
    <form onSubmit={handleSubmit}>
      <div className="row g-3">
        <div className="col-md-6">
          <label htmlFor="neptunCode" className="form-label">Neptun-kód</label>
          <input id="neptunCode" name="neptunCode" className="form-control" value={formData.neptunCode} onChange={handleChange} required />
        </div>
        <div className="col-md-6">
          <label htmlFor="roomNum" className="form-label">Szobaszám</label>
          <input id="roomNum" name="roomNum" type="number" className="form-control" value={formData.roomNum} onChange={handleChange} placeholder="Opcionális" />
        </div>
        ...
        <div className="col-md-6">
          <label htmlFor="confirmPassword" className="form-label">Jelszó újra</label>
      ...
      {error ? (
        <div className="alert alert-danger py-2 mt-3" role="alert">{error}</div>
      ) : null}
      ...
      <p className="text-center mt-4 mb-0">
        Már van fiókod?{" "}
        <Link to={APP_ROUTES.LOGIN} className="text-decoration-none">Bejelentkezés</Link>
      </p>
    </form>
  );
};
