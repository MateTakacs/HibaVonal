...
import { filterBySearch, getIssueSearchText, getOrderSearchText, groupCountBy } from "../utils/search";
...
export const useLeadMaintainerDashboard = () => {
  ...
  const loadIssues = useCallback(async () => {
    try {
      ...
      setAssignFormByIssueId((current) => {
        ...
        data.forEach((issue) => {
          next[issue.id] = issue.assignedMaintainerId ? String(issue.assignedMaintainerId) : next[issue.id] || "";
        });
      ...
      showErrorToast(createApiErrorMessage(error, "A hibajegyek betöltése nem sikerült."), "lead-issues-load");
      return [];
    } finally {
      if (mountedRef.current) {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      const data = await getLeadMaintainerOrders({ status: orderFilter || undefined });
      ...
      showErrorToast(createApiErrorMessage(error, "A rendelések betöltése nem sikerült."), "lead-orders-load");
      return [];
    } finally {
      if (mountedRef.current) {
  ...
  const loadMaintainers = useCallback(async () => {
    try {
      ...
      showErrorToast(createApiErrorMessage(error, "A karbantartók listája nem tölthető be."), "lead-maintainers-load");
      return [];
    } finally {
      if (mountedRef.current) {
  ...
  const handleSelectIssue = useCallback(async (issueId) => selectIssue(issueId), [selectIssue]);
  ...
  const handleAssignIssue = useCallback(
    async (issueId) => {
      ...
      if (!maintainerId) {
        showErrorToast("Válassz karbantartót a kiosztáshoz.", "lead-assign-validation");
        return;
      }
      ...
      try {
        ...
        showErrorToast(createApiErrorMessage(error, "A hibajegy kiosztása nem sikerült."), "lead-issue-assign");
      } finally {
        if (mountedRef.current) {
  ...
  const handleIssueStatusUpdate = useCallback(
    async (issueId, status) => {
      try {
        ...
        showErrorToast(createApiErrorMessage(error, "A hibajegy státusza nem frissíthető."), "lead-issue-status");
      } finally {
        if (mountedRef.current) {
          setMutatingIssueId(null);
        }
  ...
  const handleCreateOrder = useCallback(
    async (event) => {
      ...
      if (!orderForm.toolListId || !deliveryDateIso) {
        showErrorToast("Adj meg érvényes eszközt és szállítási dátumot.", "lead-order-validation");
        return;
      }
      ...
      try {
        ...
        showErrorToast(createApiErrorMessage(error, "A rendelés létrehozása nem sikerült."), "lead-order-create");
      } finally {
        if (mountedRef.current) {
    ...
    [loadOrders, mountedRef, orderForm.deliveryDate, orderForm.toolListId, resetOrderDeliveryDate],
  );
  ...
  const handleOrderStatusUpdate = useCallback(
    async (orderId, status) => {
      try {
        ...
        showErrorToast(createApiErrorMessage(error, "A rendelés státuszát nem sikerült frissíteni."), "lead-order-status");
      } finally {
        if (mountedRef.current) {
  ...
  const handleRefreshAll = useCallback(async () => {
    await Promise.allSettled([loadIssues(), loadOrders(), loadMaintainers(), refreshSelectedIssue()]);
