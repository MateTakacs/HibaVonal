...
export const useMaintainerDashboard = () => {
  ...
  const loadIssues = useCallback(async () => {
    try {
      ...
      const data = await getMaintainerIssues({ status: issueFilter || undefined });
      ...
      showErrorToast(createApiErrorMessage(error, "A hibajegyek betöltése nem sikerült."), "maintainer-issues-load");
      return [];
    } finally {
      if (mountedRef.current) {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      const data = await getMaintainerOrders({ status: orderFilter || undefined });
      ...
      showErrorToast(createApiErrorMessage(error, "A rendelések betöltése nem sikerült."), "maintainer-orders-load");
      return [];
    } finally {
      if (mountedRef.current) {
  ...
  const handleIssueStatusUpdate = useCallback(
    async (issueId, status) => {
      try {
        ...
        showErrorToast(createApiErrorMessage(error, "A státusz frissítése nem sikerült."), "maintainer-issue-status");
      } finally {
        if (mountedRef.current) {
  ...
  const handleOrderSubmit = useCallback(
    async (event) => {
      ...
      if (!orderForm.toolListId || !deliveryDateIso) {
        showErrorToast("Adj meg érvényes eszközt és szállítási dátumot.", "maintainer-order-validation");
        return;
      }
      ...
      try {
        ...
        showSuccessToast(response.message || "A rendelési igény rögzítve lett.");
        resetOrderDeliveryDate();
        await loadOrders();
      } catch (error) {
        showErrorToast(createApiErrorMessage(error, "A rendelési igény létrehozása nem sikerült."), "maintainer-order-create");
      } finally {
        if (mountedRef.current) {
